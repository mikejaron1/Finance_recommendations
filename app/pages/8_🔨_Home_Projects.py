...
profile = page_setup("Home Projects", "🔨")

st.title("🔨 Home Projects")
st.markdown(
    "Solar, turf and renovations, each judged two ways: what you **recoup at resale** and what you "
    "**save in cashflow** — always against the alternative of just investing the money."
)

tab1, tab2, tab3 = st.tabs(["☀️ Solar", "🌿 Turf / xeriscaping", "🛠️ Renovations"])
...
with tab1:
    ...
    with c1:
        st.markdown("**System**")
        cost = st.number_input("System cost before incentives ($)", 1_000, 500_000, 28_000, 1_000)
        size = st.number_input("System size (kW)", 1.0, 100.0, 8.0, 0.5)
        credit = st.number_input("Federal tax credit", 0.0, 0.50, 0.30, 0.01, format="%.2f",
                                 help="The Residential Clean Energy Credit. Verify the current rate and expiry — it steps down.")
        rebate = st.number_input("State/local rebate ($)", 0, 100_000, 0, 500)
    with c2:
        st.markdown("**Production & rates**")
        production = st.number_input("Annual production (kWh per kW)", 500, 2_500, 1_400, 50,
                                     help="~1,600 in Arizona, ~1,400 in California, ~1,100 in the Northeast.")
        rate = st.number_input("Electricity rate ($/kWh)", 0.01, 1.00, 0.32, 0.01,
                               help="California ~$0.32, US average ~$0.17.")
        utility_inflation = st.number_input("Utility rate inflation", 0.0, 0.20, 0.045, 0.005, format="%.3f",
                                            help="Utility rates have historically outpaced CPI — this drives most of the return.")
    with c3:
        st.markdown("**Net metering**")
        nem = st.slider("Export credit rate", 0.0, 1.0, 0.25, 0.05, format="%.0f%%",
                        help="NEM 1.0/2.0 paid full retail (100%). California's NEM 3.0 pays roughly 25%.")
        self_consumption = st.slider("Self-consumption share", 0.0, 1.0, 0.55, 0.05, format="%.0f%%",
                                     help="Power you use directly is worth full retail. Batteries push this up.")
        financed = st.checkbox("Financing the system")
...
with tab2:
    c1, c2, c3 = st.columns(3)
    with c1:
        sqft = st.number_input("Lawn area (sq ft)", 100, 100_000, 2_000, 100)
        cost_sqft = st.number_input("Install cost ($/sq ft)", 1.0, 50.0, 12.0, 0.5,
                                    help="Artificial turf ~$10-15/sq ft. Drought-tolerant xeriscaping ~$6-10.")
        rebate_sqft = st.number_input("Water district rebate ($/sq ft)", 0.0, 10.0, 2.0, 0.25)
    with c2:
        water_cost = st.number_input("Current landscape water cost ($/yr)", 0, 50_000, 900, 50)
        water_inflation = st.number_input("Water cost inflation", 0.0, 0.20, 0.05, 0.005, format="%.3f")
        lawn_maintenance = st.number_input("Lawn maintenance ($/yr)", 0, 50_000, 1_800, 100,
                                           help="Mowing, fertiliser, aeration, reseeding.")
    with c3:
        turf_maintenance = st.number_input("Turf maintenance ($/yr)", 0, 10_000, 200, 50)
        lifespan = st.number_input("Turf lifespan (years)", 5, 30, 18,
                                   help="Turf isn't permanent. The replacement cost is what kills the long-run return.")
        analysis_years = st.number_input("Analysis period (years)", 5, 40, 20)
...
with tab3:
    ...
    c1, c2, c3 = st.columns(3)
    project = c1.selectbox("Project", list(RENOVATION_ROI),
                           format_func=lambda k: RENOVATION_ROI[k]["label"])
    spec = RENOVATION_ROI[project]
    cost = c2.number_input("Your quoted cost ($)", 500, 2_000_000, int(spec["typical_cost"]), 500)
    years_until_sale = c3.number_input("Years until you sell", 1, 40, profile.planned_years_in_home)

    c1, c2, c3 = st.columns(3)
    enjoyment = c1.number_input("Annual value to you ($/yr)", 0, 100_000, 0,
                                help="What would you pay per year to have this? Be honest — this is a real benefit, just not a financial return.")
    operating = c2.number_input("Annual operating savings ($/yr)", 0, 100_000, 0,
                                help="Lower energy bills, less maintenance.")
    financed = c3.checkbox("Financing it", key="reno_fin")
