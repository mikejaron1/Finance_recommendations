...
profile = page_setup("Spending & Cash", "🧾")

st.title("🧾 Spending, Savings & Cash")
...
with tab1:
    st.markdown(
        ...
        "is processed **in memory on your machine** — nothing is uploaded or stored anywhere."
    )
    ...
    if transactions is not None and len(transactions):
        ...
        with c1:
            ...
            fig.update_layout(template="plotly_white", height=520, margin=dict(l=10, r=10, t=40, b=10),
        ...
        with c2:
            fig = go.Figure(go.Pie(labels=by_category["category"].head(10), values=by_category["total"].head(10), hole=0.5))
            fig.update_layout(template="plotly_white", height=520, margin=dict(l=10, r=10, t=40, b=10),
...
with tab2:
    ...
    if transactions is None or not len(transactions):
        ...
        manual = st.number_input("Or estimate: how much could you cut per month? ($)", 0, 100_000, 500, 50)
        monthly_savings = manual
    else:
        target_cut = st.slider("How aggressively would you cut each flexible category?", 0.1, 1.0, 0.5, 0.05,
                               format="%.0f%%")
    ...
    c1, c2 = st.columns(2)
    amount = c1.number_input("Monthly savings to model ($)", 0, 100_000, int(monthly_savings), 25)
    ret = c2.number_input("Invested at", 0.0, 0.20, profile.expected_return, 0.005, format="%.3f", key="sav_r")
...
with tab3:
    ...
    with c1:
        essential = st.number_input("Essential monthly expenses ($)", 0, 500_000,
                                    int(profile.monthly_essential_spending), 250, key="ef_e")
        current_cash = st.number_input("Current cash ($)", 0, 100_000_000, int(profile.cash), 1_000, key="ef_c")
    with c2:
        stability = st.selectbox("Job stability", ["stable", "average", "volatile"],
                                 index=["stable", "average", "volatile"].index(profile.job_stability), key="ef_s")
        earners = st.number_input("Income earners", 1, 4, profile.income_sources, key="ef_i")
    with c3:
        dependents = st.number_input("Dependents", 0, 10, profile.dependents, key="ef_d")
        apy = st.number_input("Savings APY", 0.0, 0.15, 0.042, 0.001, format="%.4f")

    c1, c2 = st.columns(2)
    self_emp = c1.checkbox("Self-employed", profile.self_employed, key="ef_se")
    disability = c2.checkbox("Have disability insurance", profile.has_disability_insurance, key="ef_di")
    ...
    near_term = st.number_input("Known expenses in the next 2 years ($)", 0, 10_000_000, 0, 1_000,
